...
class characters:
    ...
    def attributes_adjust(self):
        a=self.roles
        # Tank has more blood than the others
        if a=='tank':
           self.blood+=500
           return str(self.blood)
        # Assistant causes less damage when he attacks
        elif a=='assistant':
             self.skill1_damage-=50
             self.skill2_damage-=100
             self.skill3_damage -=100
             return str(self.skill1_damage)
        # Hunter causes more damage when he attacks
        elif a=='hunter':
             self.skill1_damage += 50
             self.skill2_damage += 80
             self.skill3_damage += 100
             return str(self.skill1_damage)
        # Mage has more magic than the others
        elif a=='mage':
             self.magic+=300
             return str(self.magic)
        # ADC has more Ex but less blood.
        elif a=='ADC':
