...
def team2_role():
  ...
  for i in range(0,len(team_two),1):
    ...
    ret+= 'Team_two '+team_two[i].name+'\t'+'is '+str(a)+',Current blood is '+str(team_two[i].blood)+' Current magic is '+str(team_two[i].magic)+' Damage of skill one is , Ex_point of the hero is '+str(team_two[i].experience_point)+'\n\n'
  return ret
